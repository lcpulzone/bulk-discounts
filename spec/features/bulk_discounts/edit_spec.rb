...
RSpec.describe 'bulk discounts edit page' do
  before :each do
    ...
    # visit "/merchants/#{@merchant___.id}/bulk_discounts/#{@discount___.id}/edit"
    # visit edit_merchant_bulk_discount_path(@merchant___.id, @discount___.id)
  end

  it 'has an edit form' do
    visit "/merchants/#{@merchant.id}/bulk_discounts/#{@discount2.id}/edit"
    ...
    expect(page).to have_button("Submit Discount Update")
  end

  it 'can successfully fill out the form' do
    visit "/merchants/#{@merchant.id}/bulk_discounts/#{@discount2.id}/edit"
    ...
    click_button "Submit Discount Update"

    expect(current_path).to eq("/merchants/#{@merchant.id}/bulk_discounts/#{@discount2.id}")
    expect(page).to have_content("Discount has been successfully updated, YAY!")
    expect(page).to have_content("40%")
  end

  it 'can successfully fill out the form by changing only one attribute' do
    visit "/merchants/#{@merchant.id}/bulk_discounts/#{@discount5.id}/edit"
    ...
    expect(current_path).to eq("/merchants/#{@merchant.id}/bulk_discounts/#{@discount5.id}")
    expect(page).to have_content("Discount has been successfully updated, YAY!")
    expect(page).to have_content(@discount5.percentage)
  ...
  it 'sad path' do
    ...
    expect(current_path).to eq("/merchants/#{@merchant.id}/bulk_discounts/#{@discount2.id}/edit")
