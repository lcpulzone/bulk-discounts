...
RSpec.describe 'Admin Merchants Index Page' do
  ...
  it "I click on the name of a merchant, which is a link that takes me to that merchant's admin show page" do
    ...
    expect(page).to have_link(@merchant6.name, href: admin_merchant_path(@merchant6.id))
  ...
  it "I can click a button to enable or disable a merchants status" do
    within("#enabled-merchant-#{@merchant1.id}") do
      ...
      @new_merchant1 = Merchant.find(@merchant1.id)
    ...
    within("#disabled-merchant-#{@merchant4.id}") do
      ...
      @new_merchant4 = Merchant.find(@merchant4.id)
      expect(@new_merchant4.status).to eq('enable')
    end
  end
  ...
  it "I see the date with the most revenue for each merchant and it looks like 'Top selling date for <merchant name> was <date with most sales>'." do
    ...
    within "#top-rev-merchants-#{@merchant2.id}" do
      expect(page).to have_content("Top selling date for #{@merchant2.name} was #{@merchant2.merchant_best_day.to_date.strftime("%m/%d/%Y")}")
    end

    within "#top-rev-merchants-#{@merchant3.id}" do
      expect(page).to have_content("Top selling date for #{@merchant3.name} was #{@merchant3.merchant_best_day.to_date.strftime("%m/%d/%Y")}")
    end

    within "#top-rev-merchants-#{@merchant5.id}" do
      expect(page).to have_content("Top selling date for #{@merchant5.name} was #{@merchant5.merchant_best_day.to_date.strftime("%m/%d/%Y")}")
    end

    within "#top-rev-merchants-#{@merchant4.id}" do
      expect(page).to have_content("Top selling date for #{@merchant4.name} was #{@merchant4.merchant_best_day.to_date.strftime("%m/%d/%Y")}")
    end
  end
end
