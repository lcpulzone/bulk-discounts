...
RSpec.describe 'invoice show page' do
  before(:each) do
    ...
    @customer7 = Customer.create!(first_name: 'Jerry', last_name: 'Basic')
    ...
    @item7 = Item.create!(name: 'Thing 7', description: 'This is the seventh thing.', unit_price: 6.3, merchant_id: @antimerchant.id)
    ...
    @invoice7 = Invoice.create!(status: 1, customer_id: @customer7.id, created_at: "2021-06-03 20:11:38.553871")
    ...
    @invoice_item13 = InvoiceItem.create!(quantity: 3, unit_price: 6.3, status: 2, invoice_id: @invoice7.id, item_id: @item7.id)
  ...
  describe 'discounts' do
    ...
    it 'has a link to the discount show page if applicable' do
      ...
      within("##{@invoice_item1.id}") do
        ...
        expect(page).to_not have_link("#{@discount3.percentage}", href: "/merchants/#{@merchant.id}/bulk_discounts/#{@discount3.id}")
      end

    visit "/merchants/#{@antimerchant.id}/invoices/#{@invoice7.id}"  
      within("##{@invoice_item13.id}") do
        expect(page).to have_content("#{@invoice_item13.quantity}")
        expect(page).to have_content("#{@invoice_item13.unit_price}")
        expect(page).to have_content("#{@invoice_item13.status_for_view}")
